...
public class ExcelService {
    ...
    public ByteArrayInputStream getScheduleForStudentForSpecifiedWeek(final long id, final int week) {
        ...
        if (maybeUser.isPresent()) {
            try (Workbook workbook = new XSSFWorkbook(); ByteArrayOutputStream out = new ByteArrayOutputStream()) {
                ...
                int rowIndex = 1;
                Map<WeekDay, List<LessonDTO>> schedule = courseService.findScheduleForWeek(week, id);
                for (Map.Entry<WeekDay, List<LessonDTO>> day : schedule.entrySet()) {
                    ...
                    Map<LessonOrder, List<LessonDTO>> lessonsByOrder = new TreeMap<>();
                    Stream.of(LessonOrder.values()).forEach(lessonOrder -> lessonsByOrder.put(lessonOrder, day.getValue().stream().filter(l -> l.getTime().equals(lessonOrder)).collect(Collectors.toList())));
                    Row row = dayRow;
                    for (Map.Entry<LessonOrder, List<LessonDTO>> lessonsOnTheSameTime : lessonsByOrder.entrySet()) {
                        ...
                        for (LessonDTO lesson : lessonsOnTheSameTime.getValue()) {
    ...
    private void writeLesson(final Row row, final LessonDTO lesson, final CellStyle style) {
        ...
        cell.setCellStyle(style);
        cell.setCellValue(lesson.getType().getType());
